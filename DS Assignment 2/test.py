...
class Stock():
    def __init__(self, name, quantity, price):
        ...
        self.symbols = self.item_name[0].upper() if name else ""


class Vending_Ui():
    def __init__(self, stock_list):
        self.stock_list = stock_list if stock_list else [Stock("", 0, 0)]
        self.username = "admin"
        self.password = "1234"

    def check_empty(self):
        if len(self.stock_list) == 1 and self.stock_list.name == "":
    ...
    def buy_menu(self):
        ...
        if self.stock_list[ans-1].quantity == 0:
            ...
            if amt > self.stock_list[ans-1].price:
                print(f"Your Change is { amt - (self.stock_list[ans-1].price)}")
                self.stock_list[ans-1].quantity -= 1
            else:
                print("Insufficient Payment Given /nTransection Canceled")
    ...
    def admin_inventory(self): 
        if input("Password: ") == self.password:
            while True:
                ...
                print("\n[1] Refill Stock (Add specific amount)")
                print("[2] Reset Stock (Max out all items)")
                print("[3] Update Price")
                print("[4] Change Item (Rename)")
                print("[5] Delete Item")
    ...
    def refill_stock(self): 
        ans = input("What Item would you like to refill /n(1, 2, 3, 4, 5, ALL):").strip().upper()
        if ans == "ALL":
            for i in self.stock_list:
                i.quantity = MAX_QTY
        elif int(ans) in [1, 2, 3, 4, 5]:
    ...
    def update_price(self): 
        ans = input("What Item's price would you like to change /n(1, 2, 3, 4, 5):").strip()
        if int(ans) in [1, 2, 3, 4, 5]:
            self.stock_list[int(ans)-1].price = float(input("New Price: "))
        else:
            print("invalid input")

    def change_item(self): 
        ans = input("What Item's name would you like to change /n(1, 2, 3, 4, 5):").strip()
        if int(ans) in [1, 2, 3, 4, 5]:
            self.stock_list[int(ans)-1].item_name = input("New Name: ")
        else:
            print("invalid input")

    def delete_item(self): 
        ans = input("What Item would you like to delete /n(1, 2, 3, 4, 5):").strip()
        if int(ans) in [1, 2, 3, 4, 5]:
...
def main():
    # Max 5 items, 20 qty
    stock_list = []
    stock_item = [("Milo", 10, 2.50), ("Coke", 0, 2.50), ("A&W", 2, 2.50), ("100+", 4, 2.50), ("", 12, 2.50)]
    for i in stock_item:
